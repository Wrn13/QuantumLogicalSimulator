...
class ExperimentRunner:
    ...
    @staticmethod
    def experiment_partial_run_instant_erasure(trotterer: TrotterGroup, initial_state: qt.Qobj, setup_circuits: List[Tuple[List[qt.Qobj], List[float], List[qt.Qobj], List[List[qt.Qobj]], List[List[float]]]]) -> List[qt.Qobj]:
        """Execute one or more circuit setups sequentially, performing erasure checks instantly.
            ...
            for state, prob, measurement_history in branched_current_states:
                for i, proj in enumerate(measurements):
                    ...
                    if proj_result > 1e-12:
                        ...
                        # If full, perform recovery after
                        if len(new_measurement_history[0]) == 2 or len(new_measurement_history[1]) == 1:
                            perform_recovery = True
            ...
            if perform_recovery:
                ...
                for state, prob, measurement_history in branched_current_states:
                    ...
                    if is_phase and len(measurement_history[0]) == 2:
                        measurement_index = measurement_history_to_int(measurement_history[0])
                        new_measurement_history = ((), measurement_history[1])
                    elif not is_phase and len(measurement_history[1]) == 1:
                        measurement_index = measurement_history_to_int(measurement_history[1])
                        new_measurement_history = (measurement_history[0], ())
                    else:
                        measurement_index = -1
