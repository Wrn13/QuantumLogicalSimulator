...
from src.quantum_logical.noisy_gate import NoisyGate
from src.quantum_logical.snail_module import SNAIL_Module
...
def noisy_qubit_partial_phase_circuit(single_qudit_time: float, two_qudit_time: float, target_state: int = 0, module: SNAIL_Module = None):
    circuit = []
    gate_time = []
    N = 4

    ideal_H_layer = qt.tensor(hadamard_operator(2), hadamard_operator(2), hadamard_operator(2), qt.qeye(2))

    # Hadamard layer
    circuit.append(ideal_H_layer)
    gate_time.append(single_qudit_time)

    # CNOT Layer
    cnot_1 = cnot_operator(num_qudits=N, dim=2, control_idx=target_state, target_idx=3, trigger=1)
    cnot_2 = cnot_operator(num_qudits=N, dim=2, control_idx=1+target_state, target_idx=3, trigger=1)

    circuit.append(module.generate_noisy_gate(target_state, 3, cnot_1))
    circuit.append(module.generate_noisy_gate(target_state+1, 3, cnot_2))
    gate_time.extend([two_qudit_time] * 2)
...
def noisy_qubit_parallel_phase_circuit(single_qudit_time: float, two_qudit_time: float, num_ancillae: int = 2, module: SNAIL_Module = None):
    """
    Docstring for qubit_parallel_phase
